	...
	class Utils {
		...
		public function getGamerTurnInformation($gamerId, $isMakeNextTurn = false) {
			...
			if (!$isCashPresent) {
				...
				$gamerTurnData->setData('cash', $results[0]['result']);
			}
		...
		// public function getCharityTurnsLeft($gamerId) {
			// $sql = "SELECT charity_turns_left FROM user_model WHERE gamer_id='$gamerId'";
			// $results = $this->dataBaseController->getter($sql);		
			// if (!$results) {
				// $this->responseCreator->setError('charity_turns_left error');
				// return null;
			// }			
			// return $results[0]['charity_turns_left'];
		// }
	}
?>